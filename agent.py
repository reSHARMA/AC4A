...
async def main() -> None:
    ...
    expedia = AssistantAgent(
        ...
        Similarly, if you need data from the user, ask for it in the following format: "data: question for the user", example: "data: Please provide the guest information, name, phone number, email etc".

        Give priority to asking for data over asking for confirmation over calling an API. 

        Always ask data for credit card and all the guest information before calling the payment API and add guest to booking APIs.

        Return "done" when the task is completed.

        **For searching and booking cruise follow these steps very carefully:**
            expedia_search_cruise -> ask user to select a cruise and cabin type -> expedia_get_cruise_info -> ask user for confirmation and the number of guests -> expedia_get_cruise_addons -> ask user to select addons -> expedia_get_cruise_policies -> ask user for confirmation -> expedia_get_cruise_payment_options -> ask user for payment option -> fetch credit card or payment source data -> expedia_pay_for_itenary -> fetch guest information -> expedia_add_guest_info -> ask user for confirmation -> done
