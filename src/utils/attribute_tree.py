...
class AttributeTree:
    ...
    def check_subtree(self, other_tree):
        def compare_subtrees(node1, node2):
            ...
            print(f"Comparing nodes: {key1} with value {value1} and {key2} with value {value2}")

            if key1 != key2:
                print(f"Keys do not match: {key1} != {key2}")
            ...
            for child1 in node1.children:
                ...
                if corresponding_child:
                    print(f"Found corresponding child for {list(child1.value.keys())[0]}")
                    if not compare_subtrees(child1, corresponding_child):
                        return False
                else:
                    print(f"No corresponding child found for {list(child1.value.keys())[0]} in node2")
                    # If no corresponding child is found in node2, continue
                    continue

            return True

        print("Starting subtree comparison")
        result = compare_subtrees(self, other_tree)
        print(f"Subtree comparison result: {result}")
        return result
