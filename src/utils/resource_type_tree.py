...
# ResourceTypeTree class to represent hierarchical resource type definitions (formerly AttributeTree)
class ResourceTypeTree:
    ...
    def print_tree(self, level=0):
        ...
        for key, value in self.value.items():
            ...
            logger.info(f"{indent}{key}: {value}{access_str}")
    ...
    def __str__(self):
        ...
        access_str = f" [access: {self.access}]" if self.access else ""
        if self.children:
            return f"{key}({value}){access_str} with {len(self.children)} children"
        else:
            return f"{key}({value}){access_str}"
    ...
    def get_tree_string(self):
        ...
        def _build_tree_string(node, level=0):
            ...
            lines.append(f"{indent}{key}: {value}{access_str}")
    ...
    def collect_schema(self) -> dict:
        ...
        schema = {}
        def _dfs(node: 'ResourceTypeTree'):
